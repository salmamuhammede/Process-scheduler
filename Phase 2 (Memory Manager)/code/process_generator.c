...
int readFile(char *filename)
{
    ...
    if (file)
    {
        ...
        while (fgets(data, 100, file))
        {
            if (data[0] == '#')
                continue;
            sscanf(data, "%d %d %d %d %d", &id, &arrival, &runtime, &priority,&memSize);
            ...
            currentprocess.memSize = memSize;
            setPCB(&currentprocess, id, arrival, runtime, priority , memSize);
            struct PCB dummy1;
            if(i>0)
            {
        ...
        setPCB(&currentprocess, -1, arrival + 1, 0, 0 ,0);
        enqueue(&ready, currentprocess, currentprocess.arrivaltime);
    }
    else
    {
